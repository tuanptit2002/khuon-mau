...
function uploadFiles(uploadFileObj, quillObj) {
  const filename = uploadFileObj.name;
  const now = new Date();
  const fileExt = getFileExtension(filename);
  const path = `gplx/${now.getFullYear()}/${now.getMonth()}/${getSlug(getFileName(filename))}-${now.getTime()}${fileExt}`;

  requestUploadImage(path, uploadFileObj).then((photoURL) => {
    const range = quillObj.getSelection();
    quillObj.insertEmbed(range.index, 'image', photoURL);
  }).catch(error => {
    console.log(error);
  });
}
